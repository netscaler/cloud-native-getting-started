...
You can deploy a Citrix ADM agent as a microservice in the Kubernetes cluster to view service graph in Citrix ADM. Follow below steps:
...
7. Specify the following parameters:
	...
	a. **Application ID** : A string id to define the service for the agent in the Kubernetes cluster and distinguish 		this agent from other agents in the same cluster. User lowercase alphanumeric characters only. Uppercase characters are not supported.
